...
import { AdminForthFilterOperators, AdminForthDataTypes, } from './types/Common.js';
...
class AdminForth {
    async refreshMenuBadge(menuItemId, adminUser) {
    ...
    async tr(msg, category = 'default', lang = 'en', params) {
    ...
    activatePlugins() {
        ...
        while (true) {
            ...
            const unactivatedPlugins = allPluginInstances.filter(({ pi: pluginInstance }) => !this.activatedPlugins.find((p) => p.pluginInstanceId === pluginInstance.pluginInstanceId));
            ...
            unactivatedPlugins.forEach(({ pi: pluginInstance, resource }, index) => {
                var _b;
              ...
              To support multiple plugin instance pre one resource, plugin should return unique string values for each installation from instanceUniqueRepresentation`);
                ...
                const pluginId = (_b = pluginInstance.pluginOptions) === null || _b === void 0 ? void 0 : _b.id;
                if (pluginId) {
    ...
    validateFieldGroups(fieldGroups, fieldTypes) {
        if (!fieldGroups)
            return;
        const allColumns = Object.keys(fieldTypes);
        fieldGroups.forEach((group) => {
            var _b;
            group.noTitle = (_b = group.noTitle) !== null && _b !== void 0 ? _b : false;
            group.columns.forEach((col) => {
                if (!allColumns.includes(col)) {
                    const similar = suggestIfTypo(allColumns, col);
                    throw new Error(`Group '${group.groupName}' has an unknown column '${col}'. ${similar ? `Did you mean '${similar}'?` : ''}`);
                }
            });
        });
    }
    validateRecordValues(resource, record, mode) {
        var _b, _c;
        ...
        for (const column of resource.columns.filter((col) => col.name in record && col.validation)) {
            const required = typeof column.required === 'object'
                ...
                : true;
            if (!required && !record[column.name])
                continue;
            let error = null;
            if ((_b = column.isArray) === null || _b === void 0 ? void 0 : _b.enabled) {
                error = record[column.name].reduce((err, item) => {
                    return err || _a.Utils.applyRegexValidation(item, column.validation);
                }, null);
            }
            else {
                error = _a.Utils.applyRegexValidation(record[column.name], column.validation);
            }
            if (error) {
                return error;
            }
        ...
        for (const column of resource.columns.filter((col) => {
            var _b;
            return col.name in record
                && ['integer', 'decimal', 'float'].includes(((_b = col.isArray) === null || _b === void 0 ? void 0 : _b.enabled) ? col.isArray.itemType : col.type)
                && (col.minValue !== undefined || col.maxValue !== undefined);
        })) {
            if ((_c = column.isArray) === null || _c === void 0 ? void 0 : _c.enabled) {
    ...
    async discoverDatabases() {
        ...
        this.config.databaseConnectors = Object.assign(Object.assign({}, this.connectorClasses), this.config.databaseConnectors);
    ...
    async getAllTables() {
        ...
        await Promise.all(Object.entries(this.connectors).map(async ([dataSourceId, connector]) => {
    ...
    async getAllColumnsInTable(tableName, requestedDataSourceId) {
        const results = {};
        if (!this.config.databaseConnectors) {
            this.config.databaseConnectors = Object.assign({}, this.connectorClasses);
        }
        await Promise.all(Object.entries(this.connectors)
            ...
            if (typeof connector.getAllColumnsInTable === 'function') {
                try {
                    ...
                    results[dataSourceId] = columns.map(column => (Object.assign(Object.assign({}, column), { isUUID: isProbablyUUIDColumn(column) })));
                }
                catch (err) {
     ...
     */
    async createResourceRecord(params) {
        var _b, _c, _d, _e;
        ...
        const recordWithVirtualColumns = Object.assign({}, record);
        // execute hook if needed
        for (const hook of listify((_c = (_b = resource.hooks) === null || _b === void 0 ? void 0 : _b.create) === null || _c === void 0 ? void 0 : _c.beforeSave)) {
            ...
            if (!resp || (typeof resp.ok !== 'boolean' && (!resp.error && !resp.newRecordId && !resp.redirectToRecordId))) {
                throw new Error(`Invalid return value from beforeSave hook. Expected: { ok: boolean, error?: string | null, newRecordId?: any, redirectToRecordId?: any }.\n` +
        ...
        // execute hook if needed
        for (const hook of listify((_e = (_d = resource.hooks) === null || _d === void 0 ? void 0 : _d.create) === null || _e === void 0 ? void 0 : _e.afterSave)) {
     ...
     */
    async updateResourceRecord(params) {
        var _b, _c, _d, _e;
        ...
        // execute hook if needed
        for (const hook of listify((_c = (_b = resource.hooks) === null || _b === void 0 ? void 0 : _b.edit) === null || _c === void 0 ? void 0 : _c.beforeSave)) {
            const resp = await hook({
                recordId,
                resource,
                record: dataToUse,
        ...
        // execute hook if needed
        for (const hook of listify((_e = (_d = resource.hooks) === null || _d === void 0 ? void 0 : _d.edit) === null || _e === void 0 ? void 0 : _e.afterSave)) {
            const resp = await hook({
                resource,
                record: dataToUse,
                updates: dataToUse,
     ...
     */
    async deleteResourceRecord(params) {
        var _b, _c, _d, _e;
        const { resource, recordId, adminUser, record, response, extra } = params;
        // execute hook if needed
        for (const hook of listify((_c = (_b = resource.hooks) === null || _b === void 0 ? void 0 : _b.delete) === null || _c === void 0 ? void 0 : _c.beforeSave)) {
            const resp = await hook({
                resource,
                record,
                adminUser,
        ...
        // execute hook if needed
        for (const hook of listify((_e = (_d = resource.hooks) === null || _d === void 0 ? void 0 : _d.delete) === null || _e === void 0 ? void 0 : _e.afterSave)) {
            const resp = await hook({
                resource,
                record,
                adminUser,
...
AdminForth.Utils = {
    ...
    applyRegexValidation(value, validation) {
        if (validation === null || validation === void 0 ? void 0 : validation.length) {
