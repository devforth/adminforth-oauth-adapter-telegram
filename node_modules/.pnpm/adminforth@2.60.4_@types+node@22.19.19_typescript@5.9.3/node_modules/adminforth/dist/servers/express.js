...
function parseCookiesString(cookiesString) {
...
function normalizeExpressRuntimeSchema(schema) {
    if (!schema) {
        return undefined;
    }
...
function normalizeExpressLayerRegexpSource(regexpSource) {
    return regexpSource
...
class ExpressServer {
    constructor(adminforth) {
        this.schemaAwareRouteRegistrationPatched = false;
        this.pendingEndpointRegistrations = [];
    ...
    setupSpaServer() {
        ...
        if (this.adminforth.runningHotReload) {
            ...
            this.expressApp.get(`${slashedPrefix}assets/*`, (req, res) => {
                res.sendFile(path.join(codeInjector.getServeDir(), replaceAtStart(req.url, prefix)), {
                    cacheControl: false,
                    // store for a year
                    headers: {
    ...
    setupWsServer() {
        ...
        wss.on('connection', async (ws, req) => {
            try {
                ...
                this.adminforth.websocket.registerWsClient(new WebSocketClient({
                    id: randomUUID(),
                    adminUser,
    ...
    serve(app) {
        ...
        const stack = (_b = (_a = this.expressApp) === null || _a === void 0 ? void 0 : _a._router) === null || _b === void 0 ? void 0 : _b.stack;
    ...
    async processAuthorizeCallbacks(adminUser, toReturn, response, extra) {
        ...
        for (const hook of listify(adminUserAuthorize)) {
            ...
            if ((resp === null || resp === void 0 ? void 0 : resp.allowed) === false || (resp === null || resp === void 0 ? void 0 : resp.error)) {
                // delete all items from toReturn and add these:
                toReturn.allowed = resp === null || resp === void 0 ? void 0 : resp.allowed;
                toReturn.error = resp === null || resp === void 0 ? void 0 : resp.error;
                break;
            }
        }
    }
    ...
    withSchema(schema, handler) {
        ...
        wrapped[EXPRESS_ROUTE_SCHEMA] = Object.assign(Object.assign({}, schema), { request: normalizeExpressRuntimeSchema(schema.request), response: normalizeExpressRuntimeSchema(schema.response) });
        return wrapped;
    }
    translatable(handler) {
        // same as authorize, but injects tr function into request
        return async (req, res, next) => {
            const tr = (msg, category, params, pluralizationNumber) => this.adminforth.tr(msg, category, req.headers['accept-language'], params, pluralizationNumber);
            req.tr = tr;
            handler(req, res, next);
        };
    }
    patchSchemaAwareRouteRegistration() {
        ...
        methods.forEach((method) => {
            var _a;
            const original = (_a = this.expressApp[method]) === null || _a === void 0 ? void 0 : _a.bind(this.expressApp);
            if (!original) {
                return;
            }
        ...
        const originalUse = (_a = this.expressApp.use) === null || _a === void 0 ? void 0 : _a.bind(this.expressApp);
        if (originalUse) {
            this.expressApp.use = ((...args) => {
                ...
                this.flattenHandlers(handlers).forEach((handler) => {
                    if (Array.isArray(handler === null || handler === void 0 ? void 0 : handler.stack)) {
                        this.registerSchemaAwareStack(handler.stack, path);
                    }
                });
    ...
    registerSchemaAwareRoute(methods, path, handlers) {
        ...
        const schema = (_a = flatHandlers.find((handler) => handler === null || handler === void 0 ? void 0 : handler[EXPRESS_ROUTE_SCHEMA])) === null || _a === void 0 ? void 0 : _a[EXPRESS_ROUTE_SCHEMA];
    ...
    registerSchemaAwareStack(stack, prefix) {
        ...
        stack.forEach((layer) => {
            ...
            const nestedStack = (_a = layer.handle) === null || _a === void 0 ? void 0 : _a.stack;
    ...
    extractLayerPath(layer) {
        ...
        const regexpSource = (_a = layer.regexp) === null || _a === void 0 ? void 0 : _a.source;
        if (typeof regexpSource !== 'string') {
            return '';
        }
        if ((_b = layer.regexp) === null || _b === void 0 ? void 0 : _b.fast_slash) {
            return '';
        }
    ...
    flattenPaths(paths) {
        ...
        const stringPaths = flattened.filter((path) => typeof path === 'string');
    ...
    endpoint(options) {
        const { method = 'GET', path, handler, noAuth = false, description, request_schema, response_schema, responce_schema, target = 'json' } = options;
        if (!path.startsWith('/')) {
            throw new Error(`Path must start with /, got: ${path}`);
        }
        const fullPath = `${this.adminforth.config.baseUrl}/adminapi/v1${path}`;
        const normalizedResponseSchema = response_schema !== null && response_schema !== void 0 ? response_schema : responce_schema;
        ...
        const expressHandler = async (req, res) => {
            ...
            const contentTypeHeader = (((_a = req.headers) === null || _a === void 0 ? void 0 : _a['content-type']) || '').toString();
            ...
            if (target === 'upload') {
                try {
                    await new Promise((resolve, reject) => {
                        this.uploadParser(req, res, (error) => {
                            if (error) {
                                reject(error);
                                return;
                            }
            ...
            const tr = (msg, category, params, pluralizationNumber) => this.adminforth.tr(msg, category, acceptLang, params, pluralizationNumber);
