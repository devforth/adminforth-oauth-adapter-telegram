...
class AdminForthAuth {
    ...
    setAuthCookie({ expireInDuration, response, username, pk }) {
        const expiresIn = expireInDuration || (process.env.ADMINFORTH_AUTH_EXPIRESIN || '24h');
    ...
    setCustomCookie({ response, payload }) {
        ...
        const brandSlug = this.adminforth.config.customization.brandNameSlug;
        response.setHeader('Set-Cookie', `adminforth_${brandSlug}_${name}=${value}; Path=${this.adminforth.config.baseUrl || '/'};${httpOnly ? ' HttpOnly;' : ''}SameSite=Strict;${sessionBased ? '' : `Expires=${new Date(Date.now() + expiryMs).toUTCString()}`}`);
    }
    getCustomCookie({ cookies, name }) {
        var _a;
        const brandSlug = this.adminforth.config.customization.brandNameSlug;
        return ((_a = cookies.find((cookie) => cookie.key === `adminforth_${brandSlug}_${name}`)) === null || _a === void 0 ? void 0 : _a.value) || null;
    }
    ...
    async verify(jwtToken, mustHaveType, decodeUser = true) {
        // read ADMINFORH_SECRET from environment if not drop error
        const secret = process.env.ADMINFORTH_SECRET;
        if (!secret) {
            throw new Error('ADMINFORTH_SECRET environment not set');
